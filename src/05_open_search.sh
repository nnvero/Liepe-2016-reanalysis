...
WORKFLOW="${WORKFLOW:-workflows/grlcl_open_search.workflow}"
WORKDIR_OUT="${WORKDIR_OUT:-results/open_search}"

mkdir -p "$WORKDIR_OUT"

# 1. Tool-created decoys + contaminants (rev_ tag). Idempotent — reuses the
#    target+decoy FASTA already built for the closed search.
bash "$SCRIPT_DIR/03_build_decoy_db.sh"

docker pull --platform linux/amd64 "$FRAGPIPE_IMG" >/dev/null

# 2. FragPipe headless — MSFragger (open search) -> Percolator -> ProteinProphet -> report.
