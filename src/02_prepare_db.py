...
def row_to_fasta(row, prefix):
  return f">sp|{prefix}-{row['ID']};{row['Gene']};{row['Domain']}\n{row['pep']}"

fasta = "\n".join(spliced.apply(lambda x: row_to_fasta(x, "LIEPE-SPLICED"), axis=1))
fasta += "\n" + "\n".join(non_spliced.apply(lambda x: row_to_fasta(x, "LIEPE-NOT-SPLICED"), axis=1))
